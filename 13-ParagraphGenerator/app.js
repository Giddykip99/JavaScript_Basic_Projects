...
//this function creates the paragraphs to be displayed
function addparagraphs(){
    let random=Math.floor(Math.random()*text.length);
    let inputvalue=parseInt(input.value);

    //if the text box is empty,bigger than 9 or less than 1
    if(isNaN(inputvalue) || inputvalue< 0 || inputvalue > text.length){
       content.innerHTML=`<P>${text[random]}</p>`;
    }else{
        let newText=text.slice(0,inputvalue);
        let neededpar=newText.map((items)=>{
            let result= `<p>${items}</p><br>`;
            return result;
        });
        let allcontent= neededpar.join(' ');
